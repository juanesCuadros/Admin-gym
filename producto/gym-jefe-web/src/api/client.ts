...
// Request Interceptor: inject Bearer token on authenticated requests
  ...
  if (axios.isAxiosError(error)) {
    ...
    if (data) {
      ...
      if (typeof data.detail === 'object' && data.detail !== null) {
        if (data.detail.codigo === 'CONFLICTO_CONCURRENCIA') {
          return `Conflicto de concurrencia: ${data.detail.mensaje || 'El registro fue modificado por otro usuario. Por favor recarga los datos antes de guardar.'}`;
        }
    ...
    if (error.response?.status === 403) {
      return 'No tienes permisos suficientes para realizar esta acción (403 Acceso Denegado).';
    }
    if (error.response?.status === 409) {
      return 'Conflicto de concurrencia (409): Los datos fueron actualizados en otra sesión concurrente. Recarga la página antes de continuar.';
    }
