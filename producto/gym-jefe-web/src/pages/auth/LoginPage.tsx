...
export const LoginPage: React.FC = () => {
  ...
  const { tenant, applyPreset, fetchPublicBranding } = useTenantTheme();
        ...
        >
          ...
          <select
            ...
            onChange={(e) => {
              ...
              const selected = TENANT_PRESETS.find((p) => p.subdominio === sub || p.id === sub);
              if (selected) {
                applyPreset(selected);
              }
              fetchPublicBranding(sub);
            }}
            style={{
          ...
          >
            {TENANT_PRESETS.map((p) => (
              <option key={p.id} value={p.subdominio} style={{ background: 'var(--bg-surface)', color: 'var(--text-primary)' }}>
                {p.nombre}
              </option>
            ))}
          </select>
