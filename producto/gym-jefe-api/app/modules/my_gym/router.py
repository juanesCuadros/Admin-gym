...
@router.get(
    "/public-branding/{subdominio}",
    response_model=PublicBrandingResponse,
    summary="Obtener branding público por subdominio sin autenticación (Pre-login)"
)
async def obtener_public_branding(
    subdominio: str,
    session: Annotated[AsyncSession, Depends(get_db_session)]
):
    """
    Retorna la información visual básica (nombre, logo_url y paleta de colores)
    asociada al subdominio del tenant, permitiendo la personalización de la pantalla de login
    y vistas públicas sin requerir token JWT.
    """
    return await MyGymService.obtener_public_branding(
        session=session,
