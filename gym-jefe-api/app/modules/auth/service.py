...
class AuthService:
    ...
    async def login(cls, session: AsyncSession, data: LoginRequest, client_ip: Optional[str] = None) -> LoginResponseDto:
        ...
        # Al no tener SET LOCAL app.gimnasio_id activo aún, current_gimnasio_id() IS NULL
        # permite evaluar intentos fallidos globales desde esta IP (mitigación cross-tenant)
        if client_ip:
        ...
        # 4. Control de fuerza bruta por cuenta específica en este gimnasio (RF-00.4)
        ...
        # 5. Buscar staff filtrando estrictamente por gimnasio_id y correo
        staff_query = text("""
        ...
        if not staff or not verify_password(data.password, staff["hash_password"]):
            # Registrar intento fallido en conexión independiente que sobrevive al rollback del request
        ...
        # 6. Validar si la cuenta está activa
        if not staff["activo"]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        # 7. Registrar éxito y actualizar último ingreso en la sesión actual
        await session.execute(
        ...
        # 8. Seed idempotente dentro de la misma sesión
        ...
        # 10. Resolver lista canónica de permisos
